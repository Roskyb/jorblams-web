import type { UIDict } from "../types";

export const about: {
	en: UIDict;
	es: UIDict;
	ja: UIDict;
	"zh-cn": UIDict;
	ar: UIDict;
} = {
	en: {
		title: "About Me",
		greeting: "Hello, I'm",
		name: "Jorge Blanco",
		role: "Full-stack Developer, Designer & Creative Technologist",
		intro: "I craft digital experiences that blend beautiful design with powerful functionality. Passionate about web technologies, user experience, and bringing ideas to life through code.",
		"cta.work": "View My Work",
		"cta.about": "About Me",
		"skills.development": "DEVELOPMENT",
		"skills.design": "DESIGN",
		"skills.tools": "TOOLS",
		"projects.title": "Featured Work",
		"projects.one": "Project One",
		"projects.one.desc": "Full-stack web application",
		"projects.two": "Project Two",
		"projects.two.desc": "Design system & components",
		"contact.title": "Let's Connect",
		"contact.description": "I'm always open to discussing new projects, creative ideas, or opportunities to be part of your vision.",
	},
	es: {
		title: "Sobre Mí",
		greeting: "Hola, soy",
		name: "Jorge Blanco",
		role: "Desarrollador Full-stack, Diseñador y Tecnólogo Creativo",
		intro: "Creo experiencias digitales que combinan diseño hermoso con funcionalidad potente. Apasionado por las tecnologías web, la experiencia de usuario y dar vida a las ideas a través del código.",
		"cta.work": "Ver Mi Trabajo",
		"cta.about": "Sobre Mí",
		"skills.development": "DESARROLLO",
		"skills.design": "DISEÑO",
		"skills.tools": "HERRAMIENTAS",
		"projects.title": "Proyectos Destacados",
		"projects.one": "Proyecto Uno",
		"projects.one.desc": "Aplicación web full-stack",
		"projects.two": "Proyecto Dos",
		"projects.two.desc": "Sistema de diseño y componentes",
		"contact.title": "Conectemos",
		"contact.description": "Siempre estoy abierto a discutir nuevos proyectos, ideas creativas u oportunidades para ser parte de tu visión.",
	},
};
